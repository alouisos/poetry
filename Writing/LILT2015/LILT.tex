...
Applying computational techniques to analyze literary style is not in itself a new idea: \citet{holmes1985analysis} discussed the benefits of quantitative measures of style; \citet{Kaplan+Blei} developed a computer program to visualize and compare the styles of different American poets; \citet{stamatatos2000automatic} used stylistic measures to automatically classify texts into different genres and authors; and \citet{kao2012computational} and \citet{dalvean2013ranking} examined the stylistic differences between professional and amateur poets. The novel contribution our work makes is that instead of generating many low-level and seemingly arbitrary features (such as the number of nouns, verbs, and commas in a poem) and using them to distinguish among various texts, we derived our measures directly based on the tenets of a specific literary style. Furthermore, we took a historical perspective to investigate the impact that this style has on later work. As a result, we hope that our findings are  interpretable and useful to scholars in the humanities, such that the relationship between a tenet---``to present an image"---and its operationalization---word-level\emph{concreteness} ratings---is sufficiently clear.
